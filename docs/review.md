...
Interactive CLI requests default to `reviewer.kind=caller_attestation`. This is
an explicit but unauthenticated caller claim; operator policy must still require
the user's decision. Automated fixtures must use `synthetic_test`. The legacy
`human` value remains readable for old records but is not emitted by the current
CLI.
...
```json
{
  ...
  "reviewer": {"kind": "caller_attestation", "id": "owner"},
  "transitions": [
    {
