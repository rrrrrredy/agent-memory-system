...
func runReviewDecide(args []string) error {
	...
	reviewerID := flags.String("reviewer", "", "human reviewer id (required)")
	...
	reason := flags.String("reason", "", "human review reason (required)")
	...
	result, err := review.Apply(store, generation.Name, review.Request{
		...
		Reviewer:      review.Reviewer{Kind: "human", ID: *reviewerID},
...
func runPromoteCandidate(args []string) error {
	...
	approverID := flags.String("approver", "", "human approver id (required)")
	...
	if status.ReviewStatus != review.StatusValidated || status.LastReviewRecordSHA256 == "" {
		return errors.New("candidate is not currently validated by a human review")
	}
	...
	result, err := promotion.Apply(store, promotion.Request{
		...
		Approver:      promotion.Approver{Kind: "human", ID: *approverID},
