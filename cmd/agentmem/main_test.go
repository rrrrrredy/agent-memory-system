...
func TestDeriveCommandDispatchAndRequiredFlags(t *testing.T) {
	tests := []struct {
		...
		{name: "missing study subcommand", args: []string{"study"}, message: "study <create|run|observe|report|verify>"},
		{name: "unknown study subcommand", args: []string{"study", "unknown"}, message: "study <create|run|observe|report|verify>"},
