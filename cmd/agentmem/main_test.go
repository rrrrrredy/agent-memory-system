...
func TestDeriveCommandDispatchAndRequiredFlags(t *testing.T) {
	tests := []struct {
		...
		{name: "missing inject adapter", args: []string{"inject"}, message: "inject <codex|claude-code|opencode|deepseek-harness>"},
		{name: "unknown inject adapter", args: []string{"inject", "unknown"}, message: "inject <codex|claude-code|opencode|deepseek-harness>"},
		...
		{name: "deepseek harness inject flags", args: []string{"inject", "deepseek-harness"}, message: "requires --root and --repo"},
		{name: "deepseek harness import flags", args: []string{"import", "deepseek-harness-events"}, message: "requires --root and --path"},
		...
		{name: "missing study subcommand", args: []string{"study"}, message: "study <register|create|run|observe|report|verify>"},
		{name: "unknown study subcommand", args: []string{"study", "unknown"}, message: "study <register|create|run|observe|report|verify>"},
		{name: "study create flags", args: []string{"study", "create"}, message: "requires --root, --repo, and --file"},
		{name: "study register flags", args: []string{"study", "register"}, message: "requires --root and --file"},
		{name: "study run flags", args: []string{"study", "run"}, message: "requires --root, --repo, --study, --task, and --codex"},
