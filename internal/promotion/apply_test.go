...
func TestSyntheticApproverKindIsPersistedAndVerified(t *testing.T) {
	fixture := newPromotionFixture(t)
	candidate := fixture.byText["Please remember that reports stay local"]
	request := promotionRequest(
		ActionPromote, "", "", fixture.generation, candidate,
	...
	request.Approver = Approver{Kind: ApproverKindSyntheticTest, ID: "synthetic-fixture"}
	result, err := Apply(fixture.store, request)
	if err != nil {
		t.Fatal(err)
	}
	if result.Revision.ApproverID != "synthetic-fixture" {
		t.Fatalf("synthetic approver id was not preserved: %+v", result.Revision)
	}
	report := Verify(fixture.store)
	if len(report.Issues) != 0 {
		t.Fatalf("synthetic promotion record did not verify: %+v", report)
	}
}
