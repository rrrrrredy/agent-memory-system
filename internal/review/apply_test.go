...
func TestSyntheticReviewerKindIsPersistedAndVerified(t *testing.T) {
	fixture := newReviewFixture(t)
	candidate := fixture.byText["Please remember that reports stay local"]
	request := validationRequest(candidate, BasisExplicitRemember, nil)
	request.Reviewer = Reviewer{Kind: ReviewerKindSyntheticTest, ID: "synthetic-fixture"}
	result, err := Apply(fixture.store, fixture.generation, request)
	if err != nil {
		t.Fatal(err)
	}
	validated, err := VerifyValidationRecord(
		fixture.store, fixture.generation, candidate.CandidateID, candidate.ContentSHA256,
		result.RecordSHA256,
	)
	if err != nil {
		t.Fatal(err)
	}
	if validated.Proof.Reviewer.Kind != ReviewerKindSyntheticTest {
		t.Fatalf("synthetic reviewer kind was not preserved: %+v", validated.Proof.Reviewer)
	}
	report := Verify(fixture.store)
	if len(report.Issues) != 0 {
		t.Fatalf("synthetic review record did not verify: %+v", report)
	}
}
